...
ID3D11Buffer* g_GPUInstanceBuffer = nullptr;
ID3D11ShaderResourceView* g_GPUInstanceSRV = nullptr;
ID3D11Buffer* g_GPUVisibleIndexBuffer = nullptr;
ID3D11ShaderResourceView* g_GPUVisibleIndexSRV = nullptr;
...
void SetupShaders()
{
    ...
    const char* instancedVS = R"(
        ...
        {
            VS_OUT output;
            uint globalId = visibleIndices[input.instanceId];
    ...
    const char* instancedPS = R"(
        ...
        StructuredBuffer<uint> visibleIndices : register(t2);
        SamplerState samplerState : register(s0);
        cbuffer InstanceData : register(b1)
        {
            struct InstanceProps
        ...
        float4 ps_main(VS_OUT input) : SV_Target0
        {
            uint idx = visibleIndices[input.id];
    ...
    const char* cullCS = R"(
        ...
        void cs_main(uint3 tid : SV_DispatchThreadID)
        {
            ...
            for (uint i = 0; i < 8; ++i)
            {
                float4 w = mul(instances[idx].world, GetLocalCorner(i));
                worldMin = min(worldMin, w.xyz);
                worldMax = max(worldMax, w.xyz);
            }
            ...
            for (int p = 0; p < 6; ++p)
            {
                float3 testPoint = worldMin;
                if (planes[p].x >= 0.0f) testPoint.x = worldMax.x;
                if (planes[p].y >= 0.0f) testPoint.y = worldMax.y;
                if (planes[p].z >= 0.0f) testPoint.z = worldMax.z;

                if (dot(float4(testPoint, 1.0f), planes[p]) < 0.0f)
                {
                    visible = false;
                    break;
                }
            ...
            {
                uint dstIndex;
                drawArgs.InterlockedAdd(4, 1, dstIndex);
                visibleIndices[dstIndex] = idx;
            }
        }
    )";
    ...
    D3DCompile(cullCS, strlen(cullCS), nullptr, nullptr, nullptr, "cs_main", "cs_5_0", flags, 0, &csBlob, &errorBlob);
    if (errorBlob) { OutputDebugStringA((const char*)errorBlob->GetBufferPointer()); RELEASE(errorBlob); }
    else { g_Device->CreateComputeShader(csBlob->GetBufferPointer(), csBlob->GetBufferSize(), nullptr, &g_CullCS); RELEASE(csBlob); }
}
...
bool CreateGPUCullingResources()
{
    ...
    D3D11_BUFFER_DESC desc = {};
    desc.ByteWidth = sizeof(GPUCullCBData);
    desc.Usage = D3D11_USAGE_DEFAULT;
    desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    hr = g_Device->CreateBuffer(&desc, nullptr, &g_GPUCullCB);
    if (FAILED(hr)) return false;

    desc = {};
    desc.ByteWidth = sizeof(GPUInstanceData) * MAX_OBJECTS;
    desc.Usage = D3D11_USAGE_DEFAULT;
    desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
    desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
    desc.StructureByteStride = sizeof(GPUInstanceData);
    hr = g_Device->CreateBuffer(&desc, nullptr, &g_GPUInstanceBuffer);
    if (FAILED(hr)) return false;
    ...
    hr = g_Device->CreateShaderResourceView(g_GPUInstanceBuffer, &srvDesc, &g_GPUInstanceSRV);
    if (FAILED(hr)) return false;

    desc = {};
    desc.ByteWidth = sizeof(UINT) * MAX_OBJECTS;
    desc.Usage = D3D11_USAGE_DEFAULT;
    desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
    desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
    desc.StructureByteStride = sizeof(UINT);
    hr = g_Device->CreateBuffer(&desc, nullptr, &g_GPUVisibleIndexBuffer);
    if (FAILED(hr)) return false;
    ...
    srvDesc.Buffer.FirstElement = 0;
    srvDesc.Buffer.NumElements = MAX_OBJECTS;
    hr = g_Device->CreateShaderResourceView(g_GPUVisibleIndexBuffer, &srvDesc, &g_GPUVisibleIndexSRV);
    if (FAILED(hr)) return false;
    ...
    hr = g_Device->CreateUnorderedAccessView(g_GPUVisibleIndexBuffer, &uavDesc, &g_GPUVisibleIndexUAV);
    if (FAILED(hr)) return false;
    ...
    hr = g_Device->CreateBuffer(&desc, nullptr, &g_GPUIndirectArgsBuffer);
    if (FAILED(hr)) return false;
...
void RenderFrame()
{
    ...
    GPUInstanceData gpuInstances[MAX_OBJECTS] = {};
    for (UINT i = 0; i < g_InstanceCount; ++i)
        XMStoreFloat4x4(&gpuInstances[i].world, g_Instances[i].world);
    g_Context->UpdateSubresource(g_GPUInstanceBuffer, 0, nullptr, gpuInstances, sizeof(GPUInstanceData) * MAX_OBJECTS, 0);

    XMVECTOR frustumPlanes[6];
    BuildFrustum(viewProj, frustumPlanes);

    GPUCullCBData cullData = {};
    for (int i = 0; i < 6; ++i)
        XMStoreFloat4(&cullData.planes[i], frustumPlanes[i]);
    ...
    g_Context->CSSetConstantBuffers(0, 1, &g_GPUCullCB);
    g_Context->CSSetShaderResources(0, 1, &g_GPUInstanceSRV);

    ID3D11UnorderedAccessView* cullUAVs[] = { g_GPUVisibleIndexUAV, g_GPUIndirectArgsUAV };
    UINT initialCounts[] = { 0, 0 };
    g_Context->CSSetUnorderedAccessViews(0, 2, cullUAVs, initialCounts);

    UINT groupCount = (g_InstanceCount + 63) / 64;
    if (groupCount > 0)
        g_Context->Dispatch(groupCount, 1, 1);

    ID3D11ShaderResourceView* nullCS_SRV[] = { nullptr };
    ID3D11UnorderedAccessView* nullCS_UAV[] = { nullptr, nullptr };
    g_Context->CSSetShaderResources(0, 1, nullCS_SRV);
    ...
    ID3D11Buffer* vsCBs[] = { nullptr, g_InstanceCB, g_ViewProjCB };
    ...
    g_Context->VSSetShaderResources(0, 1, &g_GPUVisibleIndexSRV);
    ID3D11ShaderResourceView* psSRVs[] = { g_TextureArray, g_NormalSRV, g_GPUVisibleIndexSRV };
    g_Context->PSSetShaderResources(0, 3, psSRVs);
    ...
    wchar_t title[256] = {};
    swprintf_s(title, L"3D Rendering Demo | GPU Frustum Culling | Visible: %u / %u", g_LastVisibleCount, g_InstanceCount);
    SetWindowTextW(g_MainWindow, title);
    ...
    if (g_EnableFilter)
    {
        ...
        g_Context->PSSetShaderResources(0, 1, nullPost);
    }
...
void Cleanup()
{
    ...
    RELEASE(g_CullCS);
    RELEASE(g_GPUInstanceBuffer);
    RELEASE(g_GPUInstanceSRV);
    RELEASE(g_GPUVisibleIndexBuffer);
    RELEASE(g_GPUVisibleIndexSRV);
...
int WINAPI wWinMain(_In_ HINSTANCE hInst, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int nCmdShow)
{
    ...
    {
        MessageBoxW(g_MainWindow, L"Failed to create GPU frustum culling resources", L"Error", MB_OK);
        Cleanup();
        return -1;
    }
